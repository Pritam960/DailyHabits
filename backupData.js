...
const App = {
  ...
  <div class="icon-with-text">
    <button class="icon-btn"
      style="--c1:#22d3ee; --c2:#6366f1; --c3:#a855f7;"
      onclick="App.handleBackup()">💾</button>
    <span>Save Backup Data</span>
  </div>

  <div class="icon-with-text">
    <button class="icon-btn"
      style="--c1:#facc15; --c2:#fb7185; --c3:#f97316;"
      onclick="App.handleRestore()">📂</button>
  ...
  renderDashboard() {
    ...
    while (currentDayIter <= this.data.days) {
      ...
      if (weekCount > 4 && endDay > this.data.days) {
        endDay = this.data.days;
      } else if (endDay > this.data.days) {
        endDay = this.data.days;
      }
    ...
    // Determine percentage based on view
    ...
    const currentDateString = todayObj
    ...
    let headerHTML = `
            <div class="grid-header" style="align-content:center; display:flex; flex-direction:column; justify-content:center;">
                <h1 class="premium-header-habit">
        <span class="rgb-text-effect-HABIT">HABITS</span>
    </h1> 
                
    <button onclick="App.showManageHabitsModal()" class="edit-habits-link">
        <span class="icon">✏️</span> 
        <span class="text">Edit Habits</span>
    </button>
      ...
      .map((h) => {
        ...
        return `<li style="margin-bottom: 15px;">
                        <div style="display:flex; justify-content:space-between; color: #f8fafc; font-weight:500; font-size: 0.95rem; margin-bottom: 6px;">
                            <span>${h.name}</span>
                            <span id="prio-percent-${h.id}" style="font-size:0.8rem; color:var(--primary);">${hPercent}%</span>
                        </div>
    ...
    app.innerHTML = `
                <div class="dashboard-header" style="margin-bottom: 2.5rem;">
                    <h1 style="font-size: 3.5rem; margin: 0; line-height: 1; letter-spacing: -1px;">
                         <h1 class="premium-header">
        <span class="rgb-text-effect">${this.data.month}</span> 
        <span class="rgb-text-effect secondary-glow">${this.data.year}</span>
    </h1>
                    </h1>
                    <p style="color: #94a3b8; font-size: 1rem; margin-top: 10px; margin-bottom: 25px;">
                        Track your progress and build unstoppable momentum.
                    </p>
                    <div style="display: flex; align-items: stretch; gap: 15px; max-width: 600px;">
                        <div style="flex-grow: 1; background: #1e293b; border: 1px solid #334155; border-radius: 16px; padding: 12px 20px; display: flex; align-items: center; gap: 15px;">
    ...
    <div class="avatar-inner">
        ${userInitial}
                            ...
                            <div style="display: flex; flex-direction: column;">
                                <span style="font-size: 1.1rem; font-weight: 600; color: white;">${this.currentUser}</span>
                                
                            </div>
                        </div>
                        <button onclick="App.logout()" title="Logout" 
                            style="background: linear-gradient(138deg, #ff3636 0%, #2b0c0c 70%); 
                                   border: none; border-radius: 16px; padding: 0 25px; 
                                   color: white; font-weight: 600; font-size: 0.9rem;
                                   display: flex; align-items: center; gap: 8px; cursor: pointer;
                                   box-shadow: 0 4px 12px rgba(239, 68, 68, 0.3); transition: transform 0.2s;">
                            <span>Logout</span>
                        </button>
                    </div>
                </div>
                ...
                <div class="dashboard-grid">
                    <div class="stats-sidebar">
                        ...
                        <div class="card stat-box">
                            ...
                            <h3 style="margin-bottom: 5px;">${displayLabel}</h3>
                            <div id="main-percent" class="value" style="font-size: 2.5rem; margin-bottom: 15px; color: ${this.statView === "weekly" ? "#22d3ee" : "--primary"};">${displayPercent}%</div>
    ...
    <button class="chart-toggle-btn premium-glow-btn" onclick="App.toggleGraph()">
        <span class="btn-text">${toggleBtnText}</span>
    </button>

                            </div>

                            <div id="chart-wrapper" class="${chartClass}">
                        ...
                        <div class="card">
                            ...
                            <textarea id="notes-area" placeholder="Type notes for selected day..." style="width:100%; height:100px; background:#1e293b; color:white; border:1px solid #334155; padding:10px; border-radius:6px; resize:none;" oninput="App.saveDailyNote(this.value)">${currentNote}</textarea>
                        </div>
